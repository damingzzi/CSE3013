...
void BlockDown(int sig) {
	if (CheckToMove(field, nextBlock[0], blockRotate, blockY + 1, blockX)) {
		...
		if (blockY == -1) gameOver = 1;
			...
			score+=DeleteLine(field); //블록이 아래로 내려가지 못하므로 블록을 field에 추가하고 라인을 체크
			PrintScore(score);
			DrawField();
			for (int i = 0; i < BLOCK_NUM; i++) {
				if (i == BLOCK_NUM - 1) nextBlock[i] = rand() % 7;
				else nextBlock[i] = nextBlock[i + 1];
			}
			recommend(recRoot);
			DrawNextBlock(nextBlock);
			blockRotate = 0;
...
void DrawBlockWithMODFeatures(int y, int x, int blockID, int BlockRotate) {
	DrawBlock(y, x, blockID, blockRotate, ' ');
	DrawRecommend(recommendY, recommendX, blockID, recommendR);
}
...
int recommend(RecNode* root) {
	...
	int child = 0;
	int x , y, rotate;
	int final=0, flag=0, recX, recY, recR;
	

	for (rotate = 0; rotate < 4; rotate++) {
		for (x = -2; x < WIDTH+3; x++) {
			if (!CheckToMove(root->f, nextBlock[index-1], rotate, 0, x)) {
				continue;
			}
			for (int i = 0; i < HEIGHT; i++) {
				for (int j = 0; j < WIDTH; j++) {
			...
			(root->c)[child]->lv = index;
			
			int cscore = 0;
			y = 0;
			
			while (CheckToMove((root->c)[child]->f, nextBlock[index-1], rotate, y+1, x) == 1){
				y++;
			}

			cscore += AddBlockToField((root->c)[child]->f, nextBlock[index - 1], rotate, y, x);
			cscore += DeleteLine((root->c)[child]->f);
			(root->c)[child]->score = cscore + (root->score);

			if (index < VISIBLE_BLOCKS) {
				final = recommend((root->c)[child]);
			}
			else {
				final = (root->c)[child]->score;
			}
...
int modified_recommend(RecNode *root) {
	...
	int mR = mod_Rotate(nextBlock[index - 1]);
	for (rotate = 0; rotate < mR; rotate++) {
		for (x = -2; x < WIDTH + 3; x++) {
			if (!CheckToMove(root->f, nextBlock[index - 1], rotate, 0, x)) {
				continue;
			}
...
void recommendedPlay() {
	...
	do {
		...
		blockY = -1;
		blockRotate = 0;
		DrawBlockWithMODFeatures(blockY, blockX, nextBlock[0], blockRotate);
		blockX = recommendX;
		blockY = recommendY;
